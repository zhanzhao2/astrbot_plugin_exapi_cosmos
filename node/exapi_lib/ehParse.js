...
class EhGallery {
    ...
    async _parseGalleryThumbnails(html) {
        ...
        let listImg = $("#gdt").find("div.gdtl").find("a>img");
        if (listImg.length > 0) {
            this._total = listImg.length;
            listImg.each((i, element) => { this._thumbnails[i] = $(element).attr("src"); })
            return;
        }

        let listDiv = $("#gdt").find("a>div[style]");
        this._total = listDiv.length;
        listDiv.each((i, element) => {
            let style = $(element).attr("style") || "";
            let m = style.match(/url\((.*?)\)/);
            if (m && m[1]) this._thumbnails[i] = m[1];
        })
    }
    ...
    async _parseGalleryViewImg(html) {
        ...
        if (a.length === 0) a = $("#gdt").find("a[href*=\"/s/\"]");
        a.each((i, element) => {
            let raw = $(element).attr("href") || "";
            let href = raw.match(/s\/([0-9a-z]+)\/(.*?)$/i);
            if (!href) return;
            this._viewImgHref[i] = [];
