class EhUrl {
    ...
    static viewImg(token, id, nl) {
        const q = nl ? (`?nl=${encodeURIComponent(nl)}`) : '';
        return `${this.host}/s/${token}/${id}${q}`
    }
...
class EhSearchQuery {
    ...
    _addAdvancedBetween() {
        let advancedConfig = this._config.advanced;
        if (advancedConfig.between instanceof Array && advancedConfig.between.length === 2) {
